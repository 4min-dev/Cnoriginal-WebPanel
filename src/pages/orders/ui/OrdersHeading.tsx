...
type OrdersHeadingProps = {
    ...
    handleDownloadOrdersTable: () => void,
    isDownloadProcessing: boolean,
    isPayAllReady: boolean,
    isPayPvzProcessing: boolean,
    handlePvzPay: () => void,
    canCreateOrder: boolean
}

const OrdersHeading: React.FC<OrdersHeadingProps> = ({ handleSetActiveNewOrderPopup, searchQuery, onSearchChange, handlePayAll, isPayProcessing, handleDownloadOrdersTable, isDownloadProcessing = false, isPayAllReady, isPayPvzProcessing, handlePvzPay, canCreateOrder }) => {
        ...
        <div className='flex flex-col lg:mt-[20px] lg:pr-[24px] lg:pl-[24px] mt-[32px] pr-[16px] pl-[16px]'>
            ...
            <div className='flex justify-between mt-[16px] flex-wrap lg:flex-nowrap'>
                <div className='flex lg:gap-x-[9px] lg:gap-y-[9px] gap-x-[9px] gap-y-[12px] flex-wrap lg:flex-nowrap'>
                    <Button disabled={!canCreateOrder} className='lg:w-[164px] w-[46.37%] h-[44px] order-[-1] lg:order-0 gap-[8px] text-[15px] lg:text-[16px] rounded-[10px]' clickHandler={handleSetActiveNewOrderPopup} buttonText='Новый заказ' icon={<PlusIcon />} />

                    <button type='button' className='h-[44px] w-full lg:w-[209px] items-center justify-center gap-[8px] flex font-medium text-[16px] text-[#333333] bg-white border border-[#F1F1F1] outline-none rounded-[10px] cursor-pointer disabled:opacity-[60%] disabled:cursor-default' disabled={isPayPvzProcessing} onClick={handlePvzPay}>
                    ...
                    <button type='button' className='h-[44px] w-full lg:w-[137px] items-center justify-center gap-[8px] flex font-medium text-[16px] text-[#333333] bg-white border border-[#F1F1F1] outline-none rounded-[10px] cursor-pointer disabled:opacity-[60%] disabled:cursor-default' onClick={handlePayAll} disabled={isPayProcessing || !isPayAllReady}>
                        {isPayProcessing ? 'Оплата...' : 'Оплатить все'}
                    </button>

                    <button type='button' className='h-[44px] lg:w-[195px] w-[51.12%] items-center justify-center gap-[8px] flex font-medium text-[16px] text-[#333333] bg-white border border-[#F1F1F1] outline-none rounded-[10px] cursor-pointer order-[-1] lg:order-none' onClick={handleDownloadOrdersTable} disabled={isDownloadProcessing}>
                        <svg className="lg:w-[24px] lg:h-[24px] w-[20px] h-[20px]" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
                        ...
                        {isDownloadProcessing ? 'Загрузка...' : 'Скачать таблицу'}
                    </button>
                </div>
