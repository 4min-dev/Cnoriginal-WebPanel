...
const ItemsInOrderPopup: React.FC<{ onClose: () => void }> = ({ onClose }) => {

    const itemsData: ItemObject[] = [
        {
            id: 1,
            imgUrl: '/images/headphones.png',
            track: '#VNHBVUI3487HF',
            description: 'Blutooth наушники JBL33322',
            price: 'Стоимость',
            amount: 5
        },
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 md:items-center" onClick={onClose}>
            <div className="lg:w-[661px] w-full mx-[12px] lg:mx-0 bg-white rounded-[20px] md:rounded-2xl shadow-2xl max-h-[90vh] lg:max-h-[554px] overflow-y-auto lg:p-[20px]" onClick={(e) => e.stopPropagation()}>
                <div className='flex items-center justify-between'>
                    <span className='font-semibold text-[28px] text-[#33331F]'>
                        Товары в заказе
                    </span>
                ...
                <div className='flex flex-col grow gap-[12px] mt-[24px]'>
                    {
                        itemsData && itemsData.map((item, index) => (
                            <div key={item.id} className='flex items-center gap-[8px]'>
                                <div className='h-[67px] px-[14px] flex items-center justify-center border border-dashed border-[#B9B9B94D] rounded-[16px]'>
                                    <span className='font-medium text-[28px] text-[#B3B3B3] opacity-[70%]'>
                                        {
                                            index + 1
                                        }
                                    </span>
                                ...
                                <div className='flex items-center justify-between h-[68px] px-[12px] rounded-[16px] border border-[#B9B9B94D] grow'>
                                    <div className='flex gap-[12px]'>

                                        <img src={item.imgUrl} alt={item.description} className='w-[44px] h-[44px] rounded-[8px]' />
                                    ...
                                    <div className='flex gap-[24px] items-center'>
                                        ...
                                        <button type='button' className='bg-none outline-none border border-[#B9B9B94D] rounded-[8px] w-[36px] h-[36px] flex items-center justify-center cursor-pointer'>
                    ...
                    <button type='button' className='grow h-[44px] flex items-center justify-center border-[10px] outline-none border-none bg-[#ED0028] font-medium text-[16px] text-[#FCFDFF] mt-[24px] rounded-[10px] cursor-pointer'>
                        Создать заказ
                    </button>
                </div>
            </div>
