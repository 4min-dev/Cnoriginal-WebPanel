...
const trackNumberStart: string[] = JSON.parse(
    import.meta.env.VITE_TRACK_NUMBER_PREFIXES || '[]'
)
...
const NewOrderPopup: React.FC<{
    ...
    const [selectedClient, setSelectedClient] = useState<Client | null>(user ? { ...user, id: user.id } : null)
    ...
    const hasSelectedClientPassportData = Boolean(
        selectedClient?.passport_series &&
        selectedClient?.passport_number &&
        selectedClient?.passport_issue_date
    )
    ...
    const validateTableFile = useCallback(async (file: File): Promise<CreateOrderError | null> => {
    ...
    }, [formatFileSize, validateTableFile, setUploadedFile, setError])
    ...
    const isValidTrackingNumber = (value: string) => {
        ...
        return trackNumberStart.some(prefix =>
    ...
    const handleSubmit = async () => {
        ...
        if (!isValidTrackingNumber(trackingNumber)) {
            setError({
                title: 'Неверный трек-номер',
                description: `Трек номер может начинаться только с: ${trackNumberStart
                    .map(prefix => `"${prefix}"`)
                    .join(', ')}`
            })
            return
        ...
        const orderData: NewOrder & { tables?: File[], imageFile?: File } = {
            ...
            tables: uploadedFile?.file ? [uploadedFile.file] : undefined,
            count: typeof count === 'number' ? count : 0,
            image_url: '',
            client_id: selectedClient?.id == user?.id ? null : selectedClient?.id || null,
    ...
    function handleDownloadTableExample(e: React.MouseEvent<HTMLButtonElement>): void {
        e.stopPropagation()

        const link: HTMLAnchorElement = document.createElement('a')
        link.href = '/files/tableExample.xlsx'
            ...
            <div className="lg:w-[507px] w-full mx-[12px] lg:mx-0 bg-white rounded-[20px] md:rounded-2xl shadow-2xl max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
                ...
                <div className="px-[20px] mt-[24px]">
                    <div className='flex flex-col gap-[8px]'>
                        ...
                        <div className='flex items-center gap-[4px] relative w-full'>
                            <span className='font-medium text-[13px] lg:text-[14px] text-[#B9B9B9] whitespace-nowrap'>
                                Трек-номер
                            </span>
                            ...
                            >
                                ...
                                {showTooltip && (
                                    <div className='absolute left-[22px] top-1/2 -translate-y-1/2 z-20 whitespace-wrap bg-[#333] text-white text-[12px] px-[10px] py-[6px] rounded-[8px] shadow-lg w-full max-w-[85%]'>
                                        Должен начинаться с: {trackNumberStart.join(', ')}
                                    </div>
                                )}
                            </div>
                        </div>
                    ...
                    <div className="grid grid-cols-2 gap-[12px] lg:gap-[16px] mt-[18px] lg:mt-[24px]">
                        <div>
                            ...
                            <input
                                ...
                                value={costPerGood}
                                onChange={(e) => {
                                    const digits = handleDigitsOnly(e.target.value)
                                    setCostPerGood(digits === '' ? '' : Number(digits))
                                }}
                                placeholder="¥"
                    ...
                    <div className='mt-[32px] flex flex-col gap-[16px] lg:gap-[20px]'>
                        ...
                        <div
                            className="border border-dashed border-[#B9B9B966] rounded-[16px] w-full h-[220px] flex flex-col items-center justify-center text-center bg-[#F9F9F980] transition-colors"
                        ...
                        >
                            ...
                            <button type='button' className='outline-none border border-[#F1F1F1] bg-white rounded-[10px] h-[36px] lg:w-[222px] w-[209px] flex items-center justify-center gap-[8px] lg:mt-[16px] mt-[24px] cursor-pointer' onClick={handleDownloadTableExample}>
                                <span className='font-medium lg:text-[14px] text-[13px] text-[#333333]'>
                                    Скачать пример таблицы
                                </span>

                                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16" fill="none">
                                    <path fill-rule="evenodd" clip-rule="evenodd" d="M8 1.5C8.13261 1.5 8.25979 1.55268 8.35355 1.64645C8.44732 1.74021 8.5 1.86739 8.5 2V9.79333L10.6467 7.64667C10.6924 7.59754 10.7476 7.55814 10.809 7.53081C10.8703 7.50348 10.9365 7.48879 11.0037 7.48761C11.0708 7.48642 11.1375 7.49877 11.1997 7.52392C11.262 7.54907 11.3185 7.5865 11.366 7.63398C11.4135 7.68145 11.4509 7.73801 11.4761 7.80027C11.5012 7.86253 11.5136 7.92921 11.5124 7.99635C11.5112 8.06348 11.4965 8.12969 11.4692 8.19103C11.4419 8.25236 11.4025 8.30756 11.3533 8.35333L8.35333 11.3533C8.25958 11.447 8.1325 11.4996 8 11.4996C7.8675 11.4996 7.74042 11.447 7.64667 11.3533L4.64667 8.35333C4.59754 8.30756 4.55814 8.25236 4.53081 8.19103C4.50348 8.12969 4.48879 8.06348 4.48761 7.99635C4.48642 7.92921 4.49877 7.86253 4.52392 7.80027C4.54907 7.73801 4.5865 7.68145 4.63397 7.63398C4.68145 7.5865 4.73801 7.54907 4.80027 7.52392C4.86253 7.49877 4.92921 7.48642 4.99635 7.48761C5.06348 7.48879 5.12969 7.50348 5.19103 7.53081C5.25236 7.55814 5.30756 7.59754 5.35333 7.64667L7.5 9.79333V2C7.5 1.86739 7.55268 1.74021 7.64645 1.64645C7.74022 1.55268 7.86739 1.5 8 1.5ZM2 10.5C2.13261 10.5 2.25979 10.5527 2.35355 10.6464C2.44732 10.7402 2.5 10.8674 2.5 11V12.5C2.5 12.7652 2.60536 13.0196 2.79289 13.2071C2.98043 13.3946 3.23478 13.5 3.5 13.5H12.5C12.7652 13.5 13.0196 13.3946 13.2071 13.2071C13.3946 13.0196 13.5 12.7652 13.5 12.5V11C13.5 10.8674 13.5527 10.7402 13.6464 10.6464C13.7402 10.5527 13.8674 10.5 14 10.5C14.1326 10.5 14.2598 10.5527 14.3536 10.6464C14.4473 10.7402 14.5 10.8674 14.5 11V12.5C14.5 13.0304 14.2893 13.5391 13.9142 13.9142C13.5391 14.2893 13.0304 14.5 12.5 14.5H3.5C2.96957 14.5 2.46086 14.2893 2.08579 13.9142C1.71071 13.5391 1.5 13.0304 1.5 12.5V11C1.5 10.8674 1.55268 10.7402 1.64645 10.6464C1.74021 10.5527 1.86739 10.5 2 10.5Z" fill="#222222" />
                                </svg>
                            </button>
                        </div>
                        ...
                        <div className="flex flex-col gap-[20px] lg:gap-[24px]">
                            <div className='flex flex-col gap-[8px]'>
                                ...
                                <select
                                    ...
                                    onChange={(e) => {
                                        const client = e.target.value == user?.id ? user : userClients.find(c => c.id === e.target.value)
                    ...
                    <Button disabled={isSubmitting || !selectedClient?.id || !hasSelectedClientPassportData || !thumbnailFile || uploadedFile?.status === 'error' || !trackingNumber || !description || !productUrl || !costPerGood || !count} clickHandler={handleSubmit} buttonText={isSubmitting ? 'Создание заказа...' : 'Продолжить'} className='lg:mt-[24px] mt-[16px] w-full h-[44px] mb-[20px] text-[15px] lg:text-[16px] rounded-[10px]' />
                </div>
            </div>
        </div>
    )
