...
const OrderList: React.FC<{
...
}> = ({ copyTrackNumber, dobropostOrders, deliveryOrders, searchQuery, handleSetActiveNewOrderPopup }) => {
    ...
    const matchesSearch = (order: Order): boolean => {
        ...
        const customer = order.customer_personal_data?.[0]
    ...
    const filteredDeliveryOrders = deliveryOrders.filter(matchesSearch)

    const newOrders = filteredDobropostOrders.filter(o => o.actual_bx_status === 'new')
    ...
    const pvzOrders = filteredDobropostOrders.filter(o => o.actual_bx_status === 'pvz')
    ...
    const renderOrderCard = (order: Order) => {
        ...
        return (
            <div
                key={order.id}
                className='relative rounded-[16px] border border-[#F1F1F1] bg-[#FDFDFD] grow overflow-hidden'
            >
                ...
                <div className='mt-[20px] py-[12px] border-t border-dashed border-[#E7E7E7] flex justify-between px-[16px]'>
                    <span className='text-[13px] text-[#B9B9B9] h-[16px]'>
                        {order.price ? 'Стоимость' : 'К оплате'}
                    </span>
                    <span className={`text-[13px] text-[${order.price ? '#B9B9B9' : '#ED0028'}] h-[16px]`}>
                        {order.price ? order.price.toLocaleString('ru-RU') + ' ¥' : order.cost_per_good.toLocaleString('ru-RU') + ' ₽'}
                    </span>
                </div>
            </div>
        )
        ...
        <div className='lg:pt-[32px] pt-[20px] lg:pl-[24px] pl-[16px] lg:pr-[24px] pr-[16px] pb-[50px] lg:pb-[20px] flex gap-[12px] overflow-x-scroll'>
            ...
            <div className='flex flex-col p-[20px] rounded-[16px] bg-white shadow-[0_0_25.8px_0_#0f0f2b26] min-w-[359px] max-w-[359px] h-fit'>
                <div className='flex items-center justify-between'>
                    ...
                    <button type='button' className='bg-none outline-none border-none cursor-pointer' onClick={handleSetActiveNewOrderPopup}>
            ...
            <div className='flex flex-col p-[20px] rounded-[16px] bg-white shadow-[0_0_25.8px_0_#0f0f2b26] min-w-[359px] max-w-[359px] h-fit'>
                ...
                <div className='mt-[24px] flex flex-col gap-[16px]'>
                    {checkingOrders.map(renderOrderCard)}
                </div>
            </div>
            ...
            <div className='flex flex-col p-[20px] rounded-[16px] bg-white shadow-[0_0_25.8px_0_#0f0f2b26] min-w-[359px] max-w-[359px] h-fit'>
                ...
                <div className='mt-[24px] flex flex-col gap-[16px]'>
                    {chinaStorageOrders.map(renderOrderCard)}
                </div>
            </div>
            ...
            <div className='flex flex-col p-[20px] rounded-[16px] bg-white shadow-[0_0_25.8px_0_#0f0f2b26] min-w-[359px] max-w-[359px] h-fit'>
                ...
                <div className='mt-[24px] flex flex-col gap-[16px]'>
                    {inTransitOrders.map(renderOrderCard)}
                </div>
            </div>
            ...
            <div className='flex flex-col p-[20px] rounded-[16px] bg-white shadow-[0_0_25.8px_0_#0f0f2b26] min-w-[359px] max-w-[359px] h-fit'>
                ...
                <div className='mt-[24px] flex flex-col gap-[16px]'>
                    {filteredDeliveryOrders.map(renderOrderCard)}
                </div>
            </div>
